...
const Login = () => {
    ...
    const loginHandle = (e) => {
      ...
      const loginData = { 
       ...
       axios.post("http://13.212.31.237/cs/login", loginData)
       .then(res => {
         if(res.status === 200 ) {
...
<Row >
  ...
  <Button variant="outline-warning" className="mr-5" onClick={() => history.push('./Home')}>Back to Home</Button>
</Navbar>
</Container>
<Jumbotron fluid className="patternCS">
</Jumbotron>
    ...
    <div className="card-body">
      <form className="" action="POST">
        ...
        <div className="form-group ml-auto">
            <button onClick={loginHandle} type="submit" name="signIn" className="btn btn-warning btn-block"><i className="fa fa-upload fa-fw"></i>Sign In</button>
        </div>
      </form>
    </div>
  </div>
